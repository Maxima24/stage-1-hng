...
export class StringsController {
  ...
  async uploadString(@Body() payload: UploadStringSchema) {
    if (!payload?.value) {
      throw new HttpException({ message: 'Missing "value" field' }, 400);
    }

    try {
      const result = await this.stringService.upload(payload);
      return result; // 201 automatically
    } catch (err: any) {
      if (err.message === 'Duplicate string') {
        throw new HttpException({ message: 'String already exists' }, 409);
      }
      throw err;
    }
  }
  ...
  async searchString(@Query() query: any) {
    if (query.value) {
      ...
      if (!res) throw new HttpException({ message: 'String not found' }, 404);
      return res;
    }
    ...
    if (query.is_palindrome || query.min_length || query.max_length || query.word_count || query.contains_character) {
      ...
      if (query.is_palindrome !== undefined) {
        filterPayload.is_palindrome = query.is_palindrome === 'true';
      }
